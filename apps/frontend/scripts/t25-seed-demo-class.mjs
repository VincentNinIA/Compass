...
const learnerPseudonym =
  process.env.COMPASS_TEST_LEARNER_PSEUDONYM?.trim() || "Demo";
...
let learnerAlias = classroom?.learnerAliases?.find(
  (alias) =>
    alias.pseudonym.toLocaleLowerCase("fr") ===
    learnerPseudonym.toLocaleLowerCase("fr"),
);
let learnerCookie;
if (!learnerAlias) {
  ...
  learnerCookie = readCookie(joined.response, "compass_learner_session");
  learnerAlias = joined.payload.membership?.learnerAlias;
}
const learnerAliasId = learnerAlias?.id;
...
let assignment = classroom?.assignments?.find(
  (candidate) =>
    ["scheduled", "active"].includes(candidate.status) &&
    candidate.contractHash === activity.contractHash &&
    candidate.recipientAliasIds?.includes(learnerAliasId),
);
if (!assignment) {
  ...
  assignment = assigned.payload.assignment?.assignment ?? assigned.payload.assignment;
  if (assignment) {
    assignment = {
      ...assignment,
      publication: assigned.payload.assignment?.publication,
    };
  }
}
...
if (learnerCookie) {
  ...
  assignment = membership.payload.membership?.assignments?.find(
...
if (
  ...
  publication?.schemaVersion !== "teacher_exercise_publication.v2" ||
  publication?.content?.kind !== "geometry_investigation"
...
async function request(
  path,
  { method = "GET", cookie, body, retryRateLimit = true } = {},
) {
  ...
  if (response.status === 429 && retryRateLimit) {
    ...
    const delayMs = Number.isFinite(retryAfter)
      ? Math.min(Math.max(retryAfter * 1_000 + 1_000, 1_000), 65_000)
      : 61_000;
    console.log("Production write budget reached; waiting before one safe retry.");
