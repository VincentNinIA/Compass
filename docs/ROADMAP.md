...
runtime T22 avait alors été qualifié sur la branche candidate `0c8e3f4`; son
intégration à `main` est désormais acquise par T24-C01.
...
Playwright hors live, golden 3/3 et smoke Realtime 1/1. `origin/main` contient
la qualification `2f582fd` et la PR #3 est fusionnée via `4ea885f`.
T24-C02 est close `pass` : l'accès Production est signé et expirant, les API
métier échouent fermées avant tout coût, et le WAF Vercel limite globalement
les `POST /api/*` à 6 par minute et par IP.
T24-C03 est close `pass` : `5493bd9` corrige le contraste détecté lors du
premier déploiement et la Production READY `dpl_GQtBPXN765XSqrPLyJpakyUZsfen`
sert désormais le candidat T22 protégé. Les parcours public Varignon et
Realtime passent. T25-C01 est close `pass` avec 862 tests, ses contrats stricts,
ses cascades et ses migrations PostgreSQL testées. T25-C02 est close `pass` :
classes, codes scrypt rotatifs, roster pseudonyme, adapter PostgreSQL et parcours
EN/FR passent 876 tests sur 102 fichiers et le golden navigateur 2/2. Le porteur
confirme ensuite que `math.pdf` est l'énoncé à affecter. T25-C03 est close
`pass` : publication exacte hashée, groupes et snapshots transactionnels,
...
L'amendement direct est close `pass` sur `e6a747d` et la Production
`dpl_9iyS9y7dhuy3LMJRA4qGXbX3tqbL` après smoke du vrai applet et retour accueil.
...
| T24-C01 | done | T23-C02 | Candidat T22 intégré à main et identité de release figée |
| T24-C02 | done | T24-C01 | Accès démo protégé et routes coûteuses limitées |
| T24-C03 | done | T24-C02 | Artefact T22 exact déployé et parcours public qualifié |
| T24-C04 | backlog | T24-C03 | Vidéo, fiche hackathon et dossier de soumission synchronisés |
| T25-C01 | done | T24-C03 | Contrats de données, accès, store, rétention et suppression fermés |
| T25-C02 | done | T25-C01 | Classes, codes rotatifs et roster pseudonyme isolés |
| T25-C03 | done | T25-C02 | Affectations versionnées par classe, groupe ou alias |
| T25-C04 | in_progress | T25-C03 | File élève et reprise sûre d'une affectation |
...
| FR-57 — Public demo budget | T24-C02, T24-C03, T27-C01 | Accès public sans code, rate limit WAF et secrets protégés |
