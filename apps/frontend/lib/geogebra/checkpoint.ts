...
import { initializeMinimalScene, type SceneRegistry } from "./scene";
...
export class CheckpointService {
  ...
  private rebuildCanonicalScene():
    ...
    const scene = initializeMinimalScene(this.adapter, this.registry);
    if (!scene.ok) {
  ...
  private restoreInitialRegistry() {
    this.registry.replace([
  ...
  > {
    ...
    return new Promise((resolve) => {
      result.value?.((base64) => resolve({ ok: true, value: String(base64) }));
    });
  }
  ...
  private readObjectNames():
    ...
    const result = this.adapter.withApi((api) => api.getAllObjectNames?.());
    if (!result.ok || !result.value) {
      return {
        ok: false,
        error: {
